...
# It has been modified to process real AGN host galaxy data
# Overall, it serves the similar role of roouhsc.py, except:
...
def roou():
    ...
    parser.add_argument("--source", default="{target dataset name}")
    ...
    # When using different datasets, flags in the catalogs are different.
    ...
    files_mb = []
    for f_index in range(num_filters):
        fits_path = '%s/*.fits' % (galaxy_inputs[f_index])
    ...
    for f_index in range(num_filters):
        ...
        for i in files:
            image_id = os.path.basename(i).replace('.fits', '')
            # print('\n')
            # print(image_id)
            ...
            mat_path = '%s/test/%s.npy' % (outputs[f_index], image_id)
